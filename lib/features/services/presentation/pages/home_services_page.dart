...
class _HomeServicesPageState extends State<HomeServicesPage> {
  ...
  Future<_HomeLocationResult?> _resolveCurrentLocation() async {
    try {
      ...
      return _HomeLocationResult(label: label);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _openAddProviderSheet({_ServicePro? existing}) async {
